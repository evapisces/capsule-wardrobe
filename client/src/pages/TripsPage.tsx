...
// Exported so tests can assert the values without depending on jsdom's CSSOM
// parsing `min()` / `dvh` (AC 9).
export const modalFormStyle: React.CSSProperties = {
  background: 'var(--bg-page)', borderRadius: '14px', padding: '26px',
  width: 'min(380px, calc(100vw - 32px))', border: '1px solid var(--line-strong)',
...
export default function TripsPage() {
  ...
  // Sub-breakpoint reflow points specific to this screen.
  const stackTripRows = useMediaQuery('(max-width: 479px)');
  const stackDatePair = useMediaQuery('(max-width: 399px)');

  // iOS Safari auto-zooms on focus for inputs under 16px; bump at mobile.
  const inputStyle: React.CSSProperties = {
    height: '40px', padding: '0 13px', borderRadius: '9px',
    border: '1px solid var(--line-default)', fontSize: isMobile ? '16px' : '14px',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {trips.map((trip) => (
          <button
            ...
            style={{
              textAlign: 'left', padding: '18px 22px', minHeight: '64px',
              border: '1px solid var(--line-strong)', borderRadius: '12px',
              background: 'var(--bg-raised)', cursor: 'pointer',
              display: 'flex',
              flexDirection: stackTripRows ? 'column' : 'row',
              justifyContent: 'space-between',
              alignItems: stackTripRows ? 'flex-start' : 'center',
              gap: stackTripRows ? '6px' : '12px',
          ...
          >
            ...
            <div style={{
              ...
              textAlign: stackTripRows ? 'left' : 'right',
              display: stackTripRows ? 'flex' : 'block',
              gap: stackTripRows ? '4px' : undefined,
            }}>
      ...
      {showCreate && (
        <div style={{
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: stackDatePair ? 'column' : 'row', gap: stackDatePair ? 0 : '10px' }}>
