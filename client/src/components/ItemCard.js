...
function formatLastWorn(item) {
    ...
    if (item.dormant)
        return `Not worn in ${days} days`;
    const weeks = Math.round(days / 7);
    return `Worn ${weeks} week${weeks === 1 ? '' : 's'} ago`;
}
export default function ItemCard({ item, isInActiveCapsule, onClick, maxWearCount }) {
    ...
    return (_jsxs("button", { style: cardStyle, onClick: () => onClick?.(item), "aria-label": item.name, children: [_jsxs("div", { style: { position: 'relative' }, children: [_jsx("div", { style: photoStyle, children: !item.photoUrl && '👕' }), showBadge && (_jsx("span", { style: { position: 'absolute', top: '10px', right: '10px' }, onClick: (e) => e.stopPropagation(), children: _jsx(Tooltip, { content: `In ${item.capsuleCount} capsule${item.capsuleCount === 1 ? '' : 's'}`, children: _jsx("span", { style: { ...badgeStyle, position: 'static' }, children: item.capsuleCount }) }) }))] }), _jsx("div", { style: usageBarTrack, children: _jsx("div", { style: usageBarFill }) }), _jsxs("div", { style: rowStyle, children: [_jsx("span", { style: nameStyle, children: item.name }), _jsx("span", { style: wearCountStyle, children: wearCount === 0 ? 'Never worn' : `${wearCount} wear${wearCount === 1 ? '' : 's'}` })] }), _jsx("div", { style: metaStyle, children: formatLastWorn(item) }), details && _jsx("div", { style: detailStyle, children: details })] }));
}
