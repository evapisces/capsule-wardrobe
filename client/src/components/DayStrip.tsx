...
export default function DayStrip({ days, outfitOptions, onPick }: Props) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${days.length}, 1fr)`, gap: '8px', position: 'relative' }}>
      {days.map((day) => {
            ...
            {openDate === day.date && (
              <div style={{
                ...
                {outfitOptions.length === 0 && (
                  <div style={{ padding: '9px 12px', fontSize: '12.5px', color: 'var(--ink-tertiary)' }}>No outfits yet</div>
                )}
              </div>
            )}
          </div>
