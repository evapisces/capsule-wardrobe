...
describe('GET /api/capsules/:id/board', () => {
  it('returns board items with position and off-climate flag', async () => {
    await request(app).put(`/api/capsules/${capsuleId}/board/${itemId}`).send({ x: 0.1, y: 0.1 });
    const hotItem = await prisma.closetItem.create({
      data: { closetId, name: 'Hot Item', category: 'tops', climate: 'tropical' },
    });
    await request(app).put(`/api/capsules/${capsuleId}/board/${hotItem.id}`).send({ x: 0.5, y: 0.5 });

    const res = await request(app).get(`/api/capsules/${capsuleId}/board`);
    expect(res.status).toBe(200);
...
describe('GET /api/capsules/:id/drawer', () => {
  it('excludes items already in the capsule', async () => {
    ...
    const res = await request(app).get(`/api/capsules/${capsuleId}/drawer?closetId=${closetId}`);
    expect(res.status).toBe(200);
...
describe('outfits', () => {
  it('creates an outfit from a lasso selection and supports rename/ungroup', async () => {
    ...
    await request(app).put(`/api/capsules/${capsuleId}/board/${itemId}`).send({ x: 0.1, y: 0.1 });
    await request(app).put(`/api/capsules/${capsuleId}/board/${item2.id}`).send({ x: 0.2, y: 0.1 });
    ...
    const board = await request(app).get(`/api/capsules/${capsuleId}/board`);
    expect(board.body.outfits).toHaveLength(1);
    expect(board.body.items.every((i: { outfitId: string }) => i.outfitId === outfitId)).toBe(true);

    const rename = await request(app).put(`/api/outfits/${outfitId}`).send({ name: 'Renamed' });
    expect(rename.status).toBe(200);
    expect(rename.body.name).toBe('Renamed');

    const ungroup = await request(app).delete(`/api/outfits/${outfitId}/items/${itemId}`);
    expect(ungroup.status).toBe(204);

    const boardAfter = await request(app).get(`/api/capsules/${capsuleId}/board`);
...
describe('DELETE /api/capsules/:id/board/:itemId', () => {
  it('removes the item from the board, capsule, and any outfit', async () => {
    await request(app).put(`/api/capsules/${capsuleId}/board/${itemId}`).send({ x: 0.1, y: 0.1 });
    const res = await request(app).delete(`/api/capsules/${capsuleId}/board/${itemId}`);
